1)to connect download open vpn using
# sudo apt install openvpn
then go to access page on openvpn and download your .ov file by seclect the server and click generate
then connect to the netwoek using
# sudo openvpn MahmudEg2.vo

2) Reconnaissance 
usng NMAP --> nmap tool use to scan open ports.
first scan open ports on 10.9.4.424 (machone ip) using
# nmap -sV 10.9.4.424
...
3) location directories using Gobuster: gobuster is tool use to scan directories by brouteforce from givin wordlist
...
-start bruteforceing
# gobuster dir -u http://10.10.55.30:3333 -w /usr/share/wordlists/dirbuster/directory-list-1.0.txt
...
-the directory that has an upload form page
http://10.10.55.30:3333/internal/

4) Comprpmise the Webserver
   we well use Brupsuit: it is tool use to inertupt the web browser request 

first we need to creat wordlist using
#cat > phpext.txt
.php
.php3
.php4
...
start atack
=notic that .phtml is succes

now we know that we can upload php file so we need to install PHP reverse from
...
-navigate to uplods you can guss it or brutforce using gubaster
# http://10.10.55.30:3333/internal/uploads/

then listen to the port using 
# nc -lvnp 4433
...
vist this site to know how to escleate to root
# https://gtfobins.github.io/
-Just past system SUID that you find (systemctl) and you will find all commands you need
